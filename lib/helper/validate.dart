class MyValidate {
  ...
  static String? email(String text) {
    ...
    // + @(Bắt buộc )
    if (MyValidate.nullOrEmpty(text)) {
  ...
  static bool taxNum(String value) {
    if (MyValidate.nullOrEmpty(value) || value == 0.toString()) {
      return false;
    }
  ...
  static String? empty(String text, {int? rangeFrom, int? rangeTo}) {
    ...
    if (!MyValidate.nullOrEmpty(rangeFrom)) {
      if (text.length < rangeFrom!) {
        return "Tên ít nhất phải $rangeFrom ký tự";
      }
    }
    if (!MyValidate.nullOrEmpty(rangeTo)) {
  ...
  static String? increment(String? number) {
    if (MyValidate.nullOrEmpty(number)) {
      number = '';
    }
